...
import {IWoofPair} from "./interfaces/IWoofPair.sol";
import {IWoofRouter02} from "./interfaces/IWoofRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by WoofLab teams.
 */
contract WoofZap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped tCORE (WCORE)
    IWETH public WCORE;

    // WoofRouter interface
    IWoofRouter02 public WoofRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from WoofSwap)
    ...
    // Address WoofRouter
    address private WoofRouterAddress;
     ...
     * @param _WoofRouter: address of the WoofRouter
    ...
    constructor(
        ...
        WoofRouterAddress = _WoofRouter;
        WoofRouter = IWoofRouter02(_WoofRouter);
    ...
    {
        address token0 = IWoofPair(_lpToken).token0();
        address token1 = IWoofPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IWoofPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = WoofRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = WoofRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IWoofPair(_lpToken).token0() || _token0ToZap == IWoofPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IWoofPair(_lpToken).token0() || _token1ToZap == IWoofPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IWoofPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IWoofPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IWoofPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = WoofRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = WoofRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IWoofPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "WoofRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "WoofRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        WoofRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), WoofRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(WoofRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(WoofRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = WoofRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = WoofRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = WoofRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = WoofRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = WoofRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
